...
const { serverErrorsService, excelService } = require("../../../services");
...
module.exports.getAllErrors = async (req, res, next) => {
  try {
    ...
    const { currentPage, totalPages, systemErrors } =
      await serverErrorsService.getAllErrors(page, limit);
...
module.exports.exportAllErrors = async (req, res, next) => {
  try {
    ...
    const systemErrors = await serverErrorsService.getAllErrorsList();
...
module.exports.resolveError = async (req, res, next) => {
  try {
    ...
    const systemError = await serverErrorsService.resolveError(errorId);
